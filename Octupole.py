...
class Ion():
    ...
    def InitialVelocities(self, N, mass, temp):
        ...
        for n in range(N):
            ...
            v[n] = Ion.AcceptRejectMaxwell(mass, temp)
    ...
    def AcceptRejectMaxwell(mass, temp):
...
def PseudoPotential(r, z, m):
    """Octupole pseudopotential.
    ...
    return (((charge**2) * (V_RF**2) / (m * (r_0**8) * (w**2))) * (r**6)) + (((0.5 * charge * alpha * U_end)/(r_0**2)) * (r**2)) - (((charge * alpha * U_end)/(r_0**2)) * (z**2))


def XRFPseudoForce(m, x, y):
    """Trapping pseudo-force in the x-direction due
    to the RF voltage.
...
def YRFPseudoForce(m, x, y):
    """Trapping pseudo-force in the y-direction due
    to the RF voltage.
...
def RadialStaticPseudoForce(r):
    """Defocussing pseudo-force due to the
    static endcap voltage.
...
def AxialStaticPseudoForce(z): 
    """Trapping pseudo-force in the z-direction
    due to the static endcap voltage.
...
def XRFForce(m, x, y, t):
    """Mathieu equation analogue for the octupole
    in the x-direction.
    ...
    return 2 * charge * (x**3) * (1 - (3*(y**2)/(x**2)))*V_RF*np.cos(w*t)/(r_0**4)


def YRFForce(m, x, y, t):
    """Mathieu equation analogue for the octupole
    in the y-direction.
    ...
    return 2 * charge * (y**3) * (1 - (3*(x**2)/(y**2)))*V_RF*np.cos(w*t)/(r_0**4)
...
def EvolveEuler(Ba, BaH, mode):
    ...
    for t in range(T):
        ...
        Ba.vx = Ba.vx + ((XRFForce(Ba.mass, Ba.x, Ba.y, t*dt) + RadialStaticPseudoForce(Ba.x) + XCoulombForceBa)/Ba.mass)*dt
        Ba.vy = Ba.vy + ((YRFForce(Ba.mass, Ba.x, Ba.y, t*dt) + RadialStaticPseudoForce(Ba.y) + YCoulombForceBa)/Ba.mass)*dt
        Ba.vz = Ba.vz + ((AxialStaticPseudoForce(Ba.z) + ZCoulombForceBa)/Ba.mass)*dt
        Ba.x = Ba.x + Ba.vx*dt
        Ba.y = Ba.y + Ba.vy*dt
        Ba.z = Ba.z + Ba.vz*dt

        BaH.vx = BaH.vx + ((XRFForce(BaH.mass, BaH.x, BaH.y, t*dt) + RadialStaticPseudoForce(BaH.x) + XCoulombForceBaH)/BaH.mass)*dt
        BaH.vy = BaH.vy + ((YRFForce(BaH.mass, BaH.x, BaH.y, t*dt) + RadialStaticPseudoForce(BaH.y) + YCoulombForceBaH)/BaH.mass)*dt
...
def LaserCool(Ba, t):
    """Laser cool ions with 6 randomly shuffeled lasers
    along the x, y, z axes.

    Args:
        Ba (obj): object containing positions and velocities
...
def FirstLaser(Ba, n, f_0):
    """Absorb a photon in the Z-direction.
    
    Args:
        Ba (obj): object containing positions and velocities
        of ion species to be cooled
        
        n (int): index of the ion to be cooled
        
        f_0 (float): laser frequency
    """
...
def ThirdLaser(Ba, n, f_0):
    """Absorb a photon in the XY-direction.
...
def PlotLorentzian():
    ...
    plt.savefig("Lorentzian VS Frequency.pdf")
    plt.show()
...
def PlotSpeeds(v_Ba, v_BaH):
    ...
    Args:
        v_Ba, v_BaH (float): arrays containing
    ...
    ax1.hist(v_Ba, bins = 20, facecolor = pantone, alpha = 0.75, density = True, range = (0, 40))
    #ax1.axvline(np.sqrt(np.mean(v_Ba**2)), color = 'k', linestyle = 'dashed', linewidth = 2, label = "RMS Velocity")

    v_Ba = np.delete(v_Ba, np.argwhere(v_Ba > 5))
...
def PlotHydrogenTemperatures(v_BaH):
    ...
    Args:
        v_BaH (float): array containing
        Cartesian velocities of barium
        hydride ions for the last
        secular period of the simulation,
        respectively.
    """ 
    ...
    ax.hist((10**3)*temp_H, bins = 20, facecolor = pantone, alpha = 0.75, density = True, range = (0, 50))
...
def PlotPositions(BaPositions, BaHPositions):
    ...
    BaX, BaY, BaZ = (10**3)*BaPositions
    BaHX, BaHY, BaHZ = (10**3)*BaHPositions

    figure, (ax1, ax2) = plt.subplots(1, 2, figsize = (12,5), gridspec_kw={'width_ratios': [3, 2]})

    #XZ-plane
    ax1.plot(BaZ, BaX, 'o', markersize = 6, mec = 'k', mfc = pantone, label = '$Ba^+$')
    ax1.plot(BaHZ, BaHX, 'o', markersize = 6, mec = 'k', mfc = 'w', label = '$BaH^+$')

    #XY-plane
    ax2.plot(BaY, BaX, 'o', markersize = 6, mec = 'k', mfc = pantone)
    ax2.plot(BaHY, BaHX, 'o', markersize = 6, mec = 'k', mfc = 'w')
    ...
    ax1.axis([-2.4, 2.4, -1.2, 1.2])
    ax2.axis([-1.2, 1.2, -1.2, 1.2])

    ax1.legend(loc = "upper right", prop={'size': 18}, markerscale = 2)
    plt.savefig("Final Positions.pdf")
    plt.show()


def PlotZoomIn(BaPositions, BaHPositions):
    ...
    #XZ-plane
    ax1.plot(BaZ, BaX, 'o', markersize = 12, mec = 'k', mfc = pantone, label = '$Ba^+$')
    ax1.plot(BaHZ, BaHX, 'o', markersize = 12, mec = 'k', mfc = 'w', label = '$BaH^+$')

    ax1.set_ylabel("X [$\mu m$]", **font, size = 20)
    ax1.set_xlabel("Z [$\mu m$]", **font, size = 20)
    ...
    ax1.axis([-0.4, 0.4, -0.4, 0.4])

    ax1.legend(loc = "upper right", prop={'size': 20})
    
    #XY-plane
    ax2.plot(BaY, BaX, 'o', markersize = 12, mec = 'k', mfc = pantone, label = '$Ba^+$')
    ax2.plot(BaHY, BaHX, 'o', markersize = 12, mec = 'k', mfc = 'w', label = '$BaH^+$')

    ax2.set_ylabel("X [$\mu m$]", **font, size = 20)
    ax2.set_xlabel("Y [$\mu m$]", **font, size = 20)
    ...
    ax2.axis([-2.8, -2, -0.4, 0.4])
    
    plt.savefig("Zoom-In Octupole.pdf")
    plt.show()
...
def CoulombCoupling(Ba_pos, BaH_pos, v_Ba, v_BaH):
    ...
    PE_Ba = PseudoPotential(np.sqrt(Ba_pos[0]**2 + Ba_pos[1]**2), Ba_pos[2], m_Ba)
    PE_BaH = PseudoPotential(np.sqrt(BaH_pos[0]**2 + BaH_pos[1]**2), BaH_pos[2], m_BaH)
...
f = 0.1 * 10**6 # [MHz], RF frequency
w = 2*np.pi*f
r_0 = 0.0075 # [m], radius of the trap
U_end = 1 # [V], endcap DC voltage
V_RF = 40 # [V], RF voltage amplitude

# Simulation parameters
N_Ba = 500 # number of Ba+ particles
N_BaH = 500 # number of BaH+ particles
particles = N_Ba+N_BaH
R = 0.80*10**(-3) # [m], radius of the sphere over which the particles are uniformly distributed
